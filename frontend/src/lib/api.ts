...
export const insightsApi = {
  get: (deviceId: string) =>
    apiFetch<Insight>(`/api/v1/insights/${deviceId}`),
};
...
export const integrationsApi = {
  ...
  connect: (vendor: string, apiKey: string) =>
    apiFetch<{ vendor: string; status: string }>(`/api/v1/integrations/${vendor}/connect`, {
      method: "POST",
...
export const provisioningApi = {
  ...
  deleteDeviceById: (id: string) =>
    apiFetch<{ deleted: string }>(`/api/v1/devices/${id}`, { method: "DELETE" }),
  renameDevice: (id: string, name: string) =>
    apiFetch<SavedDevice>(`/api/v1/integrations/devices/${id}`, {
      method: "PATCH",
      body: JSON.stringify({ name }),
    }),
  commissionMatter: (data: { setup_code: string; property_id: string; room_id: string }) =>
    apiFetch<Record<string, unknown>>("/api/v1/integrations/matter/commission", {
      method: "POST",
      body: JSON.stringify(data),
    }),

  scanMatter: () => apiFetch<{ devices: ScannedDevice[] }>("/api/v1/integrations/scan/matter")
...
export const shellyDeviceApi = {
  control: (deviceId: string, command: "turn_on" | "turn_off") =>
    apiFetch<{ success: boolean; state: boolean }>(`/api/v1/devices/${deviceId}/control`, {
      method: "POST",
...
export const matterApi = {
  command: (deviceId: string, command: string, value: unknown) =>
    apiFetch<Record<string, unknown>>(`/api/v1/devices/matter/${deviceId}/command`, {
      method: "POST",
