...
function ChartTooltip({ active, payload, label }: { active?: boolean; payload?: { value: number; name: string }[]; label?: string }) {
  ...
  return (
    <div className="bg-surface border border-border rounded-lg px-3 py-2 text-xs">
      ...
      {payload.map((p, i) => (
        <p key={i} className="font-mono text-text font-medium">{p.value.toFixed(2)} kWh</p>
      ))}
    </div>
  );
}

export function EnergyChart({ data, height = 200 }: { data: { label: string; value: number }[]; height?: number }) {
  const chartData = data.map((d) => ({ name: d.label, "Energy (kWh)": d.value }));
  return (
    <div style={{ height }}>
