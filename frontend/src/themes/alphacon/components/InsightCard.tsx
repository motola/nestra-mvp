...
const SEVERITY_CONFIG: Record<InsightSeverity, { border: string; bg: string; text: string; icon: typeof CheckCircle }> = {
  info: { border: "border-l-green", bg: "bg-green-bg", text: "text-green", icon: CheckCircle },
  warning: { border: "border-l-amber", bg: "bg-amber-bg", text: "text-amber", icon: AlertTriangle },
  ...
  return (
    <div className="bg-surface border border-border rounded-xl p-5">
      <div className="flex items-center gap-2 mb-3">
        ...
        {insight?.cached && <span className="font-mono text-xs text-text-3">cached</span>}
      </div>
      ...
      {insight && (() => {
        const { border, bg, text, icon: Icon } = SEVERITY_CONFIG[insight.severity];
        return (
          <div className={cn("border-l-4 rounded-r-lg p-3 flex gap-2.5", border, bg)}>
