...
async def _build_portfolio_context(settings: Settings) -> str:
    from services.property_service import list_properties
    from services.alert_service import list_active_alerts

    try:
        props, alerts = await asyncio.gather(
            list_properties(settings),
            list_active_alerts(settings),
        )
    ...
    for p in props:
        ...
        lines.append(f"- **{p.name}** ({p.address}) — {p.device_count} devices, status: {status_label}")

    if alerts:
        lines.append("\n## Active alerts\n")
        for a in alerts:
            lines.append(f"- [{a.severity.upper()}] {a.property_name} / {a.device_name}: {a.message}")

    return "\n".join(lines)


@router.post("/")
async def chat(request: ChatRequest, settings: SettingsDep):
    """Stream a portfolio-aware chat response via SSE."""
    portfolio_context = await _build_portfolio_context(settings)
    ...
    system_prompt = (
        ...
        "be specific. If asked to take an action you can't perform, say so and suggest what the user can do manually.\n\n"
        + portfolio_context
    )
