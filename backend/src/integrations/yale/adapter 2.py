"""Yale adapter — normalizes Yale devices to unified Device model."""
...
class YaleAdapter:
    ...
    async def fetch_devices(
        ...
        *,
        ...
        """Fetch Yale devices and normalize to Device objects.

        Mock implementation — returns empty list. Real implementation
        would call Yale API and parse devices.
        """
        # TODO: Call Yale API, parse devices, build Device objects with create_device_data()
        return []

    async def fetch_state(self, device: Device) -> Device:
        """Refresh device state from Yale API.

        Mock implementation — returns device unchanged.
        """
        # TODO: Call Yale API to get device state
        return Device(
            id=device.id,
    ...
    async def execute(self, device: Device, command: str, params: dict[str, object]) -> bool:
        """Execute control command on Yale device.
