...
class LifxAdapter:
    ...
    async def fetch_devices(
        ...
        *,
        ...
        Requires LIFX API token stored in integration credentials.
        ...
        try:
            ...
            if command == "turn_on":
                async with httpx.AsyncClient(
                    ...
                    response = await client.post(f"{LIFX_API_URL}/lights/{selector}/turn/on")
                    response.raise_for_status()
                    return True
                ...
                async with httpx.AsyncClient(
                    ...
                    response = await client.post(f"{LIFX_API_URL}/lights/{selector}/turn/off")
                    response.raise_for_status()
                    return True
                ...
                logger.warning(f"Unknown LIFX command: {command}")
                return False
        except Exception as e:
