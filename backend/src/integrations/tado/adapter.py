"""Tado adapter — normalizes Tado devices to unified Device model."""
...
class TadoAdapter:
    ...
    async def fetch_devices(
        ...
        *,
        ...
        """Fetch Tado devices and normalize to Device objects.

        Mock implementation — returns empty list. Real implementation
        would call Tado API and parse devices.
        """
        # TODO: Call Tado API, parse devices, build Device objects with create_device_data()
        return []

    async def fetch_state(self, device: Device) -> Device:
        """Refresh device state from Tado API.

        Mock implementation — returns device unchanged.
        """
        # TODO: Call Tado API to get device state
        return Device(
            id=device.id,
    ...
    async def execute(self, device: Device, command: str, params: dict[str, object]) -> bool:
        """Execute control command on Tado device.
